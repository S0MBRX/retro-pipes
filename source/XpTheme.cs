...
class XpForm : Form {
    ...
    protected override void OnPaint(PaintEventArgs e) {
        ...
        using(var font=new Font("Trebuchet MS",11,FontStyle.Bold)) {
            TextRenderer.DrawText(g,"Retro Pipes - Screen Saver Settings",font,new Point(35,7),Color.FromArgb(0,37,121));
            TextRenderer.DrawText(g,"Retro Pipes - Screen Saver Settings",font,new Point(34,6),Color.White);
        }
        // XP-style group frames. Controls are laid out within these sections.
        using(var p=new Pen(Color.FromArgb(172,168,153))) { g.DrawRectangle(p,18,239,369,341); g.DrawRectangle(p,397,239,375,341); }
        using(var p=new Pen(Color.White)) { g.DrawLine(p,19,581,387,581); g.DrawLine(p,398,581,772,581); }
    }
}
...
class XpButton : Button {
    ...
    protected override void OnPaint(PaintEventArgs e) {
        ...
        TextRenderer.DrawText(g,Text,Font,new Rectangle(3,pressed?2:0,Width-6,Height),Enabled?XpTheme.Ink:SystemColors.GrayText,TextFormatFlags.HorizontalCenter|TextFormatFlags.VerticalCenter);
        if(Focused&&ShowFocusCues) ControlPaint.DrawFocusRectangle(g,new Rectangle(5,5,Width-10,Height-10));
    }
}
class XpCheckBox : CheckBox {
    public XpCheckBox() { SetStyle(ControlStyles.UserPaint|ControlStyles.AllPaintingInWmPaint|ControlStyles.OptimizedDoubleBuffer,true); }
    protected override void OnPaint(PaintEventArgs e) {
        ...
        TextRenderer.DrawText(g,Text,Font,new Rectangle(20,0,Width-20,Height),Enabled?ForeColor:SystemColors.GrayText,TextFormatFlags.Left|TextFormatFlags.VerticalCenter);
...
class XpSlider : Control {
    ...
    public XpSlider() { Height=38; TabStop=true; SetStyle(ControlStyles.UserPaint|ControlStyles.AllPaintingInWmPaint|ControlStyles.OptimizedDoubleBuffer,true); }
